...
bot = Bot(token)
dp = Dispatcher(bot) # Исправлено: инициализация Dispatcher
r = sr.Recognizer()
...
async def start_command(message: types.Message):
    await message.answer('Здравствуйте! Выберите следующее действие:', reply_markup=keyboard) # Добавлена клавиатура

@dp.message(F.voice) # Исправлено: фильтр для голосовых сообщений
async def convert_audio_to_text(message: types.Message):
    ...
    try:
        ...
        text = r.recognize_google(audio, language='ru-RU') #ru-RU лучше распознает
